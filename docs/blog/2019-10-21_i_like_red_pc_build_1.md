...
/dev/disk1 (synthesized):
   ...
   1:                APFS Volume Macintosh HD            330.6 GB   disk1s1
   2:                APFS Volume Preboot                 48.3 MB    disk1s2
   3:                APFS Volume Recovery                510.4 MB   disk1s3
   4:                APFS Volume VM                      3.2 GB     disk1s4

/dev/disk2 (external, physical):
   #:                       TYPE NAME                    SIZE       IDENTIFIER
   0:     Apple_partition_scheme                        *31.5 GB    disk2
   1:        Apple_partition_map                         4.1 KB     disk2s1
   2:                  Apple_HFS                         4.1 MB     disk2s2
$ diskutil unmount /dev/disk2 || diskutil unmountDisk /dev/disk2
disk3 was already unmounted or it has a partitioning scheme so use "diskutil unmountDisk" instead
Unmount of all volumes on disk3 was successful
# NB:
# * We are using `/dev/rdisk2` instead of `/dev/disk`, this is to reduce the copy time.
# * On Mac OS X the argument `bs` should `1M` instead of `1m` as is found in Linux.
# * Press CTRL+T once you run `dd` to get a progress update.
$ time sudo dd if=/Users/mbana/Downloads/pc_build_1/linux/ubuntu-18.04.3-desktop-amd64.iso of=/dev/rdisk2 bs=1M
Password:
1986+1 records in
1986+1 records out
2082816000 bytes (2.1 GB, 1.9 GiB) copied, 146.478 s, 14.2 MB/s
sudo dd  of=/dev/rdisk2 bs=1M  0.03s user 1.14s system 0% cpu 2:30.16 total
$ diskutil eject /dev/rdisk2 && diskutil eject /dev/disk2
...
<!-- If you went ahead and managed to boot linux from the Ubuntu Live USB, great! Perhaps your BIOS is already up-to-date. In my case with the

[AMD Ryzen 7 3700X + Ryzen 9 3900X Offer Incredible Linux Performance But With A Big Caveat](https://www.phoronix.com/scan.php?page=article&item=ryzen-3700x-3900x-linux&num=1)

Go to [SUPPORT FOR PRESTIGE X570 CREATION](https://www.msi.com/Motherboard/support/PRESTIGE-X570-CREATION) -->
